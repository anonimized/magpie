...
Theorem mult_distributive_r: forall (n m p : nat), mult (plus n m) p = plus (mult n p) (mult m p).
Proof.
  intros m n p.
  rewrite mult_comm.
  rewrite mult_distributive_l.
  rewrite mult_comm.
  pattern (mult p n); rewrite mult_comm.
  reflexivity.
Qed.

Lemma mult_assoc : forall a b c : nat, mult (mult a b) c = mult a (mult b c).
Proof.
  intros a b c.
  induction a.
  simpl.
...
Theorem prod_of_pows : forall m n a : nat, mult (pow a m) (pow a n) = pow a (plus m n).
Proof.
  intros m n a.
  induction m.
  simpl.
